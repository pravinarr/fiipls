...
public class CreateTestDataSet extends Configured implements Tool {

	DataSetBean bean ;
	public void execute(DataSetBean bean) throws Exception {
		this.bean = bean;
		String[] args1 = { "" };
		int res = ToolRunner.run(new Configuration(), new CreateTestDataSet(bean), args1);
	}
	
	public CreateTestDataSet(DataSetBean b){
		this.bean = b;
	}
	...
	public int run(String[] args) throws Exception {
		...
		FileInputFormat.setInputPaths(job, new Path("hdfs://" + bean.getHdfsPath()+":"+bean.getHdfsPort()+""+bean.getTestpath()));
		FileOutputFormat.setOutputPath(job, new Path("hdfs://" + bean.getHdfsPath()+":"+bean.getHdfsPort()+""+bean.getTestDirectory()));
...
//		FileInputFormat.setInputPaths(job, new Path("hdfs://HadoopMaster:9000/input/actual/dataset.csv"));
	//	FileOutputFormat.setOutputPath(job, new Path("hdfs://HadoopMaster:9000/output_ext4/dataset"));
		...
		job.setJarByClass(CreateTestDataSet.class);
